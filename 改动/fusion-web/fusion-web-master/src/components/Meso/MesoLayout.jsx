...
import AntiFraudPanel from '../Micro/AntiFraudPanel';
...
function ConstituentDrilldown({ target, onBack }) {
  ...
  const [antiFraudTarget, setAntiFraudTarget] = useState(null);
  ...
  if (antiFraudTarget) {
    return (
      <AntiFraudPanel
        symbol={antiFraudTarget.code}
        name={antiFraudTarget.name}
        onBack={() => setAntiFraudTarget(null)}
      />
    );
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>· 成分股当日行情（点击行可查看反诈分析）</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--fs-sm)', minWidth: 580 }}>
            ...
            <tbody>
              {sorted.map((c, idx) => (
                <tr
                  key={c.code}
                  onClick={() => setAntiFraudTarget({ code: c.code, name: c.name })}
                  style={{
                    cursor: 'pointer',
                    background: idx === 0 ? 'var(--shadow-glow)' : {},
                  }}
                >
