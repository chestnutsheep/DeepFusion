...
def pm_spot_prices(
    ...
    return normalize_price_df(df, column_map, source="akshare", currency="CNY", limit=limit, float_format="%.2f",
                              date_unit=str,
                              indicator_map={"macd": "MACD", "dif": "DIF", "dea": "DEA", "kdj_k": "KDJ.K",
                                             "kdj_d": "KDJ.D", "kdj_j": "KDJ.J", "rsi": "RSI", "boll_u": "BOLL.U",
                                             "boll_m": "BOLL.M", "boll_l": "BOLL.L"})
...
def pm_etf_holdings(
    ...
    if df is None or df.empty:
        return format_error_csv("empty nbs_dictionary", "akshare", fallback=metal)
    df = df.tail(limit)
    return df.to_csv(index=False, float_format="%.2f")
...
async def pm_composite_diagnostic(
    ...
    if metal.lower() == "gold":
        sge_symbol, intl_symbol, metal_cn = "Au99.99", "XAU", "黄金"
    elif metal.lower() == "silver":
        sge_symbol, intl_symbol, metal_cn = "Ag99.99", "XAG", "白银"
    else:
        return "不支持的金属类型，仅支持: gold, silver"
    if ctx:
