...
def nbs(kw, ik, lb, freq="MM"):
    ...
    try:
        ...
        print(f"  {lb}: {int(s.index[0])}-{int(s.index[-1])}");
        return s
...
for lb, s in all_nbs.items():
    if "PPI" in lb:
        proc[lb] = np.log((s / 100.0).cumprod() * 100).clip(lower=-10)
    else:
        proc[lb] = np.log(s.clip(lower=1e-6))
...
# Global + HP
gdp_y = gdp.resample("YE").mean();
gdp_y.index = gdp_y.index.year
oil_y = oil.resample("YE").mean();
...
def cf_bp(s, lo=30, hi=60):
    ...
    sos = butter(4, [2 / hi, 2 / lo], btype="band", output="sos", fs=1.);
    v = s.values - np.nanmean(s.values)
...
ax[0].axhline(0, color="gray", lw=.5, ls="--");
...
if len(old_k) >= 2:
    ...
    if len(Xo) > 5:
        ...
        so = (pd.Series(pco.flatten(), index=Xo.index) - pd.Series(pco.flatten(), index=Xo.index).mean()) / pd.Series(
            pco.flatten(), index=Xo.index).std()
...
if len(new_k) >= 2:
    ...
    if len(nc) >= 3:
        ...
        if len(Xn) > 3:
            ...
            sn = (pd.Series(pcn.flatten(), index=nc) - pd.Series(pcn.flatten(), index=nc).mean()) / pd.Series(
                pcn.flatten(), index=nc).std()
            ax[1].plot(nc, sn, "g-", lw=2, label="New Eco")
ax[1].axhline(0, color="gray", lw=.5, ls="--");
...
ax[2].axhline(0, color="gray", lw=.5, ls="--");
...
ax[3].axhline(0, color="gray", lw=.5, ls="--");
ax[3].legend(fontsize=8);
ax[3].grid(alpha=.3)
ax[3].set_title("D: Medium Cycle (HP)");
ax[3].set_xlabel("Year")
fig.tight_layout();
fig.savefig(OUT / "kondratiev_enhanced.png", dpi=150)
print(f"\n  Saved {OUT / 'kondratiev_enhanced.png'}")

lg = glo.dropna().iloc[-1];
lc = ch.dropna().iloc[-1] if len(ch.dropna()) > 0 else 0
